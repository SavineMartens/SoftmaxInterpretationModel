...
if __name__ == "__main__":
    ...
    if test == 'AM':
        ...
        desired_temp_values = [0.001, 0.003, 0.009, 0.027, 0.081, 0.243, 0.729, 2.187, 6.561]
        fixed_sigma = 0.027 #0.081
        fixed_temp = 0.009 #0.027
        ...
        for file in all_files_EH:
            if f'temp_{temp}_' in file and f'sigmaSF_{fixed_sigma}_' in file:
                ...
                if wo_R: # remove -80 dB and y_val
                    data_EH['dB_list'] = data_EH['dB_list'][data_EH['dB_list'] > -79]
                    data_EH['y_soft_RTmax'] = data_EH['y_soft_RTmax'][data_EH['dB_list'] > -79]
    ...
    for temp in desired_temp_values:
        for file in all_files_NH:
            if f'sigmaSF_{fixed_sigma}_' in file and f'temp_{temp}_' in file:
                ...
                if wo_R: # remove -80 dB and y_val
                    data_NH['dB_list'] = data_NH['dB_list'][data_NH['dB_list'] > -79]
                    data_NH['y_soft_RTmax'] = data_NH['y_soft_RTmax'][data_NH['dB_list'] > -79]
    ...
    figHamRT_vs_RTmax, axes = plt.subplots(2, 2, figsize=(15, 7), sharex=True, sharey=True)
    axes = axes.flatten()
    # NH subplot 1 en 2
    for sigma in desired_sigma_values:
        for file in all_files_NH:
            if f'sigmaSF_{sigma}_' in file and f'temp_{fixed_temp}_' in file:
                ...
                if wo_R: # remove -80 dB and y_val
                    data_NH['dB_list'] = data_NH['dB_list'][data_NH['dB_list'] > -79]
                    data_NH['y_Hamacher_RT'] = data_NH['y_Hamacher_RT'][data_NH['dB_list'] > -79]
                    data_NH['y_Hamacher_RTmax'] = data_NH['y_Hamacher_RTmax'][data_NH['dB_list'] > -79]
    ...
    # EH subplot 3 en 4
        for file in all_files_EH:
            if f'sigmaSF_{sigma}_' in file and f'temp_{fixed_temp}_' in file:
                ...
                if wo_R: # remove -80 dB and y_val
                    data_EH['dB_list'] = data_EH['dB_list'][data_EH['dB_list'] > -79]
                    data_EH['y_Hamacher_RT'] = data_EH['y_Hamacher_RT'][data_EH['dB_list'] > -79]
                    data_EH['y_Hamacher_RTmax'] = data_EH['y_Hamacher_RTmax'][data_EH['dB_list'] > -79]
  ...
  # figure 4 subplots: softmax RT varying temp, fixed sigma, NH vs EH
    ...
    for temp in desired_temp_values:
        for file in all_files_EH:
            if f'temp_{temp}_' in file and f'sigmaSF_{fixed_sigma}_' in file:
                ...
                if wo_R: # remove -80 dB and y_val
                    data_EH['dB_list'] = data_EH['dB_list'][data_EH['dB_list'] > -79]
                    data_EH['y_soft_RT'] = data_EH['y_soft_RT'][data_EH['dB_list'] > -79]
                sigma = data_EH['sigma_SF']
    ...
    figSoftRT.savefig(f'{output_dir}Softmax_RT_Psychometric_Curves_temp_{fixed_temp}_sigma_{fixed_sigma}_norm_{norm_bool}.png')
    ...
    for file in all_files_NH:
        if f'sigmaSF_{fixed_sigma}_temp_{fixed_temp}_' in file:
            ...
            if wo_R: # remove -80 dB and y_val
                data_NH['dB_list'] = data_NH['dB_list'][data_NH['dB_list'] > -79]
                data_NH['y_soft_RT'] = data_NH['y_soft_RT'][data_NH['dB_list'] > -79]
                data_NH['y_soft_RTmax'] = data_NH['y_soft_RTmax'][data_NH['dB_list'] > -79]
